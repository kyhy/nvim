...
local foldtext_modified = function()
  local lines = '[' .. (vim.v.foldend - vim.v.foldstart + 1) .. small_l .. ']'
  local first = ({ vim.api.nvim_buf_get_lines(0, vim.v.foldstart - 1, vim.v.foldstart, true)[1]:gsub(' *', '', 1) })[1]
  local dashes = ({ vim.v.folddashes:gsub('-', '--'):gsub('-', '  ') })[1]
  local formatted = string.sub(dashes, 5)
  return formatted .. first .. middot .. middot .. lines
end

return foldtext_modified
