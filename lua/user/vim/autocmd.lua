...
local autocmd = function(name, pattern, cmd, opts)
  ...
  if cmd_type == 'function' then
    local key = user.util.get_key_for_fn(cmd, user.g.autocommand_callbacks)
    user.g.autocommand_callbacks[key] = cmd
    cmd = 'lua user.g.autocommand_callbacks.' .. key .. '()'
